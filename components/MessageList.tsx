...
export default memo<MessageListProps>(function MessageList({
  ...
  const { data: olderMessages, refetch } = useQuery(
  ...
  // this list is using reverse order so that the latest message is at the bottom
  // as requested from the server
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4 p-4 flex flex-col-reverse">
      ...
      <div className="flex justify-center pt-[4em]">
        {hasMore && (
          <Button variant="outline" onClick={handleLoadOlderMessages}>
            Load older messages
          </Button>
        )}
      </div>
